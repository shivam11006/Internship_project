...
function AssignedCases({ refreshTrigger, onNavigateToChat, onScheduleCall }) {
  ...
  // Download attachment logic - Uses blob URL if available, fallback to nothing
  const handleDownloadAttachment = (attachment) => {
    if (attachment.url) {
      // If we have a blob URL (from base64 conversion), use it to download
  ...
  return (
    <div className="assigned-cases-container">
                    ...
                    try {
                      ...
                      if (fullCaseDetails) {
                        ...
                        setSelectedCase(prev => ({
                          ...
                          status: prev.status,
                          id: prev.id, // match ID
                          evidence: processedEvidence // Use 'evidence' for the processed list with blob URLs
                        }));
                      }
                    } catch (err) {
      ...
      {/* Enhanced Details Modal - Modern Layout */}
            ...
            <div style={{ overflowY: 'auto', maxHeight: 'calc(90vh - 180px)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', minHeight: '400px' }}>
                ...
                <div style={{ padding: '24px', backgroundColor: '#f9fafb' }}>
                  ...
                  {/* Evidence & Documents Section - USING 'evidence' ARRAY FROM STATE (which has Blob URLs) */}
                  <section>
                    ...
                    {selectedCase.evidence && selectedCase.evidence.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {selectedCase.evidence.map((attachment, index) => (
                          <div
                            key={index}
                          ...
                          >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: '600', color: '#111827', fontSize: '0.875rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{attachment.fileName || attachment.name}</div>
            ...
            <div style={{ padding: '20px 24px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: '12px', backgroundColor: 'white' }}>
              {selectedCase.status === 'pending' && (
                <>
                  ...
                  <button
                    ...
                    style={{ padding: '10px 20px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 6px rgba(16, 185, 129, 0.2)' }}
                  >
                    ✓ Accept Case
                  </button>
                </>
              ...
              {(selectedCase.status !== 'pending') && (
                <button
                  ...
                  style={{ padding: '10px 20px', backgroundColor: '#f3f4f6', color: '#374151', border: '1px solid #e5e7eb', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}
                >
                  Close
                </button>
              )}
